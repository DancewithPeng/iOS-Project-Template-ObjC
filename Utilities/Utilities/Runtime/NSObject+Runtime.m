...
+ (NSInvocation*)swizzleMethod:(SEL)origSel withBlock:(id)block error:(NSError**)error {
    ...
    NSString *blockSelectorString = [NSString stringWithFormat:@"_block_%@_%p", NSStringFromSelector(origSel), block];
    ...
    [self swizzleMethod:origSel withMethod:blockSel error:nil];
    
    return origInvocation;
}

+ (NSInvocation*)swizzleClassMethod:(SEL)origSel withBlock:(id)block error:(NSError**)error {
    NSInvocation *invocation = [GetClass((id)self) swizzleMethod:origSel withBlock:block error:error];
